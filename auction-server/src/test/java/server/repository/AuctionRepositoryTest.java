...
public class AuctionRepositoryTest {
    ...
    public static void setUp() {
        // Set test mode in DatabaseConnection if supported, otherwise assume testing on main DB or local copy.
        // For simplicity, we test the existing configuration. Ideally, we should mock the connection or use an in-memory DB.
        repository = new AuctionRepository();
        itemRepository = new ItemRepository();
        
        // Cần đảm bảo có Item trong bảng items trước khi tạo Auction
        setupTestData();
    }
    ...
    private static void setupTestData() {
       try (Connection conn = DatabaseConnection.getInstance().getConnection();
            ...
            stmt.executeUpdate("DELETE FROM auctions WHERE status = 'TEST_STATUS'");
            stmt.executeUpdate("DELETE FROM items WHERE name LIKE 'Test Item%'");
    ...
    public void testUpdateBid() {
        // Assume we have an auction from previous test, or we find one
         int auctionId = -1;
         try (Connection conn = DatabaseConnection.getInstance().getConnection();
             Statement stmt = conn.createStatement();
             var rs = stmt.executeQuery("SELECT id FROM auctions LIMIT 1")) {
